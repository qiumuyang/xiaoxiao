from .metric import CharMetric, FontFormatError, FontMetric
...
def optional_font_path(ttfname: str):
    import os
    from pathlib import Path

    # check different platform system fonts paths
    if os.name == "nt":
        var_path = [
            "%WINDIR%\\Fonts",
        ...
        sys_path = [
            Path(os.path.expandvars(path)).expanduser() for path in var_path
        ]
    elif os.name == "posix":
        sys_path = [
...
class TTFMetric(FontMetric):
    ...
    def __init__(self, ttfname, log=None):
        ...
        ttfname = optional_font_path(ttfname) or ttfname
        ff = open(ttfname, "rb")
    ...
    def readFontMetrics(self, ff):
        ...
        def switchTable(tableTag):
            if tableTag not in list(tables.keys()):
                raise TTFFormatError("Required table " + tableTag +
                                     " missing in TrueType file")
        ...
        if magic != 0x5F0F3CF5:
            raise TTFFormatError(
                "Magic number in 'head' table does not match the spec")
        skip(ff, 2)
        ...
        for i in range(0, numRecords):
            ...
            if platformID == 3 and encodingID == 1:
                if languageID in englishCodes or nameID not in list(
                        uniNames.keys()):
        ...
        if subtableOffset is None:
            ...
            if subtableOffset is None:
                raise TTFFormatError(
                    "Cannot use font '%s': no known subtable in 'cmap' table" %
                    self.fullname)
            else:
                if self.log:
                    self.log.write(
                        "WARNING: font '%s' is a symbolic font - Unicode mapping may be unreliable\n"
                        % self.fullname)
        ...
        if tableFormat != 4:
            raise TTFFormatError("Unsupported format in 'cmap' table: %d" %
                                 tableFormat)
        ...
        glyphIdArray = []
        for i in range(0, remainingLength // 2):
        ...
        for i in range(0, segCount):
            for c in range(startCounts[i], endCounts[i] + 1):
                ...
                gid = 0
                if rangeOffsets[i]:
                    idx = c - startCounts[i] + rangeOffsets[i] // 2 - (
                        segCount - i)
        ...
        if self.indexToLocFormat == 0:
            ...
            raise TTFFormatError(
                "Invalid indexToLocFormat value (%d) in 'head' table" %
                str(self.indexToLocFormat))
