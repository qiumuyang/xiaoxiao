...
def drawImage(node, output):
    ...
    startElement(
        output, "metrics", SVGMathNS, "svgmath:", {
...
def default_draw(node, output):
    pass


def draw_math(node, output):
...
def draw_mphantom(node, output):
    pass


def draw_none(node, output):
    pass
...
def draw_mprescripts(node, output):
    pass


def draw_mstyle(node, output):
    draw_mrow(node, output)


def draw_mfenced(node, output):
    draw_mrow(node, output)
...
def draw_mfrac(node, output):
    ...
    if node.getProperty("bevelled") == "true":
        drawTranslatedNode(node.enumerator, output, 0,
        ...
        drawTranslatedNode(
            node.enumerator, output, node.ruleWidth +
            (node.width - 2 * node.ruleWidth - node.enumerator.width) *
            enumalign, node.enumerator.height - node.height)
        drawTranslatedNode(
            node.denominator, output, node.ruleWidth +
            (node.width - 2 * node.ruleWidth - node.denominator.width) *
            denomalign, node.depth - node.denominator.depth)
    ...
    if node.ruleWidth:
        if node.getProperty("bevelled") == "true":
            ...
            ruleX = (node.width + node.enumerator.width -
                     node.denominator.width) / 2.0
            if eh < dh:
        ...
        drawLine(output, node.color, node.ruleWidth, x1, y1, x2, y2,
                 {"stroke-linecap": "butt"})
...
def draw_msqrt(node, output):
    ...
    attrs = {
        ...
        "10",
        "d":
        ("M %f %f L %f %f L %f %f L %f %f L %f %f L %f %f L %f %f L %f %f L %f %f"
         % (x1, y1, x2a, y2a, x3a, y3a, x3b, y3b, x2b, y2b, x2c, y2c, x3, y3,
            x4, y4, x5, y5))
    }
...
def draw_msub(node, output):
    drawScripts(node, output)


def draw_msup(node, output):
    drawScripts(node, output)


def draw_msubsup(node, output):
    drawScripts(node, output)


def draw_mmultiscripts(node, output):
    drawScripts(node, output)
...
def drawScripts(node, output):
    ...
    for i in range(len(node.prewidths)):
        ...
        if i < len(node.presubscripts):
            ...
            drawTranslatedNode(presubscript, output,
                               offset - presubscript.width,
                               subY - adjustment(presubscript))
        if i < len(node.presuperscripts):
            presuperscript = node.presuperscripts[i]
            drawTranslatedNode(presuperscript, output,
                               offset - presuperscript.width,
                               superY - adjustment(presuperscript))
    ...
    for i in range(len(node.postwidths)):
        if i < len(node.subscripts):
            ...
            drawTranslatedNode(subscript, output, offset,
                               subY - adjustment(subscript))
        if i < len(node.superscripts):
            superscript = node.superscripts[i]
            drawTranslatedNode(superscript, output, offset,
                               superY - adjustment(superscript))
        offset += node.postwidths[i]


def draw_munder(node, output):
    drawLimits(node, output)


def draw_mover(node, output):
    drawLimits(node, output)


def draw_munderover(node, output):
    drawLimits(node, output)
...
def drawLimits(node, output):
    ...
    drawBox(node, output)
    drawTranslatedNode(node.base, output, (node.width - node.base.width) / 2,
                       0)
...
def draw_mtr(node, output):
    pass


def draw_mlabeledtr(node, output):
    pass
...
def draw_mtable(node, output):
    ...
    for r in range(len(node.rows)):
        ...
        for c in range(len(row.cells)):
            ...
            if cell is not None and cell.content is not None:
                ...
                if cell.colspan > 1:
                    cellWidth = sum(
                        [x.width for x in node.columns[c:c + cell.colspan]])
                    cellWidth += sum([
                        x.spaceAfter
                        for x in node.columns[c:c + cell.colspan - 1]
                    ])
                else:
                    cellWidth = column.width
                hadjust = (cellWidth - cell.content.width) * alignKeywords.get(
                    cell.halign, 0.5)
                ...
                if cell.rowspan > 1:
                    cellHeight = sum([
                        x.height + x.depth
                        for x in node.rows[r:r + cell.rowspan]
                    ])
                    cellHeight += sum([
                        x.spaceAfter for x in node.rows[r:r + cell.rowspan - 1]
                    ])
                else:
                ...
                if cell.valign == "top":
                    ...
                    vadjust = (cell.content.height - cell.content.depth +
                               cellHeight) / 2 - row.height

                drawTranslatedNode(cell.content, output, hshift + hadjust,
                                   vshift + vadjust)
    ...
    def drawBorder(x1, y1, x2, y2, linestyle):
        ...
        if linestyle == "dashed":
            ...
            extrastyle = {
                "stroke-dasharray":
                ("%f,%f" % (node.lineWidth * 7, node.lineWidth * 3)),
                "stroke-dashoffset": ("%f" % (node.lineWidth * dashoffset))
            }
        else:
            extrastyle = None
        drawLine(output, node.color, node.lineWidth, x1, y1, x2, y2,
                 extrastyle)
    ...
    for c in range(len(node.columns) - 1):
        ...
        for r in range(len(node.rows)):
            ...
            cell = row.cells[c]
            if cell is None or cell.content is None: continue
            for j in range(r, r + cell.rowspan):
...
def drawSVGText(node, output):
    ...
    attrs = {
        ...
        "text-anchor":
        "middle",
        "x": ("%f" % ((node.width + node.leftBearing - node.rightBearing) / 2 /
                      node.textStretch)),
        "y": ("%f" % (-node.textShift))
    }
