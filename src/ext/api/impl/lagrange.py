...
class LagrangeAPI(API):
    async def set_emoji_reaction(
        ...
        await self.bot.set_group_reaction(
            group_id=group_id, message_id=message_id, code=emoji, is_add=True
        )
    ...
    async def unset_emoji_reaction(
        ...
        await self.bot.set_group_reaction(
            group_id=group_id, message_id=message_id, code=emoji, is_add=False
        )
