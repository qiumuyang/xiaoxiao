...
class LLOneBotAPI(API):
    async def set_emoji_reaction(
        ...
        await self.bot.set_msg_emoji_like(message_id=message_id, emoji_id=int(emoji))

    async def unset_emoji_reaction(
        self, group_id: int, message_id: int, emoji: str
    ) -> None:
        await self.bot.unset_msg_emoji_like(message_id=message_id, emoji_id=int(emoji))
