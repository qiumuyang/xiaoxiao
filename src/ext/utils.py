...
async def get_user_name(
    event: MessageEvent | Reply, *, group_id: int | None = None, bot: Bot | None = None
) -> str:
    ...
    if group_id:
        return await get_group_member_name(group_id=group_id, user_id=user_id, bot=bot)
    if isinstance(event, GroupMessageEvent):
        return await get_group_member_name(
            group_id=event.group_id, user_id=user_id, bot=bot
        )
