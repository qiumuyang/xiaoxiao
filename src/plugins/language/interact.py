...
class RandomResponse:
    ...
    async def response(cls, group_id: int, message: Message) -> Message | None:
        since = datetime.now() - timedelta(seconds=cls.RECENT_MESSAGE_WINDOW_SECONDS)
        received_messages = await RMT.find(
            group_id=group_id, handled=False, since=since
        )
        ...
        messages = sorted(
            zip(received_messages + sent_messages, is_recv), key=lambda x: x[0].time
        )
        ...
        if i < cls.RECENT_MIN_INTERVAL_MUTE:
            return
        messages = [_InteractMessage(r, m.content) for m, r in messages] + [
            _InteractMessage(True, message)
        ]
        respond = [
            cls.repeat_respond,
            cls.keyword_respond,
    ...
    async def repeat_respond(
        ...
        if consecutive >= cls.RP_BREAK_MIN_CONSECUTIVE:
            prob = (
                cls.RP_BREAK_PROB_MIN
                + (consecutive - cls.RP_BREAK_MIN_CONSECUTIVE) * cls.RP_BREAK_PROB_INC
            )
            prob = min(prob, cls.RP_BREAK_PROB_MAX)
            if random.random() < prob:
                return Message(
                    cls.RP_RE_BREAK_TEXT
                    if repeated_message.extract_plain_text() == cls.RP_BREAK_TEXT
                    else cls.RP_BREAK_TEXT
                )
    ...
    async def keyword_respond(
        ...
        if random.random() < cls.KW_AFTER_PROB:
            ...
            results = Keyword.search(
                [query], corpus, corpus_kw, threshold=cls.KW_SIM_THRESHOLD
            )
            if results:
