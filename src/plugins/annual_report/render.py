...
class AnnualReportRenderer:
    ...
    EMOJI_FONT = FontFamily.of(
        regular=SegUIEmoji, embedded_color=True, scale=0.85, baseline_correction=True
    )
    MAIN_FONT = FontFamily.of(
        regular=NotoSansHansRegular, bold=NotoSansHansBold, fallbacks=EMOJI_FONT
    )
    ...
    CAPTION_STYLE = TextStyle(font=NotoSansHansLight, size=14, color=Palette.GRAY)

    TITLE_TEMPLATE = "{name} · {year}群聊年度报告"
    ACTIVE_DAYS_TEMPLATE = "今年，你在群内活跃了 <b>{active_days}</b> 天"
    MESSAGE_COUNT_TEMPLATE = (
        "在这些日子里，你一共发送了 <b>{num_messages}</b> 条消息，"
        "排名第 <b>{message_rank}</b>"
    )
    MOST_MESSAGE_TEMPLATE = (
        "你在 <b>{month}月{day}日</b> 发了<b>{most_message}</b>"
    ...
    TALKATIVE_DAYS_TEMPLATE = (
        "获得了 <b>{talkative_days}</b> 次群龙王，排名第 <b>{talkative_rank}</b>"
    )
    TALKATIVE_DAYS_FALLBACK = "你还没有获得过群龙王"
    POPULAR_SENTENCE_TEMPLATE = (
        "你最喜欢说的一句话是“<b>{sentence}</b>”\n"
        "过去的一年里足足说了 <b>{times}</b> 次"
    )
    NO_MESSAGE_FALLBACK = "这一年还没有发送过消息\n未语亦有痕，或待璀璨时"

    GROUP_ACTIVE_DAYS_TEMPLATE = "今年，群内活跃了 <b>{active_days}</b> 天"
    GROUP_MESSAGE_COUNT_TEMPLATE = (
        "在这些日子里，<b>{active_users}</b> "
        "位群友一共发送了 <b>{num_messages}</b> 条消息"
    )
    GROUP_MOST_USER_TEMPLATE = (
        "<b>{month}月{day}日</b>，群内共有 <b>{most_user}</b> 人发言"
    )
    GROUP_MOST_MESSAGE_TEMPLATE = (
        "<b>{month}月{day}日</b>共发送 <b>{most_message}</b> 条消息"
    )
    GROUP_COMMENTS = ["历历瞬间 令人难忘", "真是温暖的大家庭", "和你们聊天，不怕冷场"]
    GROUP_POPULAR_SENTENCE_TEMPLATE = (
        "过去的一年里，<b>{users}</b> 位群友说了<b>{times}</b>次“<b>{sentence}</b>”"
    )
    GROUP_POPULAR_SENTENCE_EXT_DESC = "此外，群友们还经常说："
    GROUP_POPULAR_SENTENCE_EXT = (
        "{items:[\n]“<bs>{{sentence}}</bs>” <s>（{{users}} 人 {{times}} 次）</s>}"
    )
    ...
    TALKATIVE_DAYS_FALLBACK_TEXT = Paragraph.of(
        TALKATIVE_DAYS_FALLBACK, style=MAIN_STYLE
    )
    NO_MESSAGE_FALLBACK_TEXT = Paragraph.of(
        NO_MESSAGE_FALLBACK, style=MAIN_STYLE, margin=Space.of_side(0, 40)
    )

    @classmethod
    def render_popular_sentence_extra(
        cls, sentences: list[tuple[str, int, int]], max_width: int
    ):
        if not sentences:
            return None
        max_items = 20
    ...
    @classmethod
    async def render_group(cls, group: GroupStatistics, group_id: int, group_name: str):
        ...
        comp_basic_info2 = Paragraph.from_template(
            ...
            values=dict(
                active_users=group.active_users, num_messages=group.num_messages
            ),
            default=cls.MAIN_STYLE,
            styles=cls.STYLES,
            alignment=Alignment.CENTER,
        ...
        comp_trend = cls._render_message_trend(
            group.message_each_month, group.message_each_day
        )
        ...
        comp_most_user = Paragraph.from_template(
            ...
            values=dict(
                month=most_user_date.month, day=most_user_date.day, most_user=most_user
            ),
            default=cls.MAIN_STYLE,
            styles=cls.STYLES,
            alignment=Alignment.CENTER,
        ...
        comp_comment = Paragraph.of(
            text=random.choice(cls.GROUP_COMMENTS),
            style=cls.MAIN_STYLE,
            alignment=Alignment.CENTER,
            max_width=cls.MAX_WIDTH,
        )
        ...
        if group.popular_sentences:
            ...
            comp_popular_sentence = Paragraph.from_template(
                ...
                values=dict(users=top_users, times=top_times, sentence=top_sentence),
                default=cls.MAIN_STYLE,
                styles=cls.STYLES,
                alignment=Alignment.CENTER,
                max_width=cls.MAX_WIDTH,
        ...
        if len(group.user_messages) > 5:
            comp_top_message_user = await cls._render_top_users(group.user_messages)
            ratio = (
                sum(
                    v
                    for _, v in sorted(
                        group.user_messages.items(), key=lambda x: -x[1]
                    )[:3]
                )
                / group.num_messages
            )
        ...
        if len(group.user_talkative_days) > 5:
            ...
            ratio = (
                sum(
                    v
                    for _, v in sorted(
                        group.user_talkative_days.items(), key=lambda x: -x[1]
                    )[:3]
                )
                / group.active_days
            )
    ...
    async def render_user(
        cls, user: UserStatistics, user_id: int, user_name: str, group_id: int
    ) -> RenderObject:
        comp_width_reserve = Spacer.of(width=cls.MAX_WIDTH)
        comp_footer = cls._render_footer()
        year = datetime.strptime(cls.ANNUAL_STATISTICS_END, "%Y-%m-%d").year
        ...
        comp_basic_info2 = Paragraph.from_template(
            ...
            values=dict(num_messages=user.num_messages, message_rank=user.message_rank),
            default=cls.MAIN_STYLE,
            styles=cls.STYLES,
            alignment=Alignment.CENTER,
            max_width=cls.MAX_WIDTH,
        ...
        comp_trend = cls._render_message_trend(
            user.message_each_month, user.message_each_day
        )
    ...
    def _render_title(cls, name: str, year: int):
        return Paragraph.from_template(
            ...
            default=TextStyle(
                font=cls.TITLE_FONT, size=cls.TITLE_FONT_SIZE, color=cls.TITLE_COLOR
            ),
            alignment=Alignment.CENTER,
            max_width=cls.MAX_WIDTH,
    ...
    async def _render_avatar(
        cls, *, user_id: int = -1, group_id: int = -1, size: int = -1
    ):
        if user_id != -1:
    ...
    def _colorize_month_trend(cls, v: float, min: float, max: float):
        return Palette.natural_blend(cls.MONTH_COLOR1, cls.MONTH_COLOR2, v / max)
    ...
    def _render_message_trend(cls, month: list[int], day: list[int]):
        ...
        chart_combine = Stack.from_children(
            (chart_month, chart_day), border=cls.TREND_BORDER
        )
        chart_combine_resize = Image.from_image(
            chart_combine.render().resize(
    ...
    async def _render_top_users(cls, users: dict[int, int]):
        ...
        comp_top = Container.from_children(
            [top[1], top[0], top[2]],
    ...
    def _render_footer(cls):
        return Paragraph.of(
            f"* 数据起止：{cls.ANNUAL_STATISTICS_BEGIN} ~{cls.ANNUAL_STATISTICS_END}",
    ...
    def _render_horizontal_line(cls, width: int):
        return Image.from_image(
            RenderImage.from_pil(PILImage.new("RGBA", (width, 1), cls.HOR_LINE_COLOR))
        )
