...
async def _(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    Usage:
        {cmd}                                           -  显示列表总览
        {cmd} {Op.ADD}|{Op.REMOVE}`<列表名>`             -  添加/删除列表
        {cmd} `<列表名>` {Op.ADD}`<内容>` ...            -  添加内容到指定列表
        ...
        * 包含指令字符({" ".join(
                           ('`' + str(op) + '`' for op in Op if str(op))
          )})且会引起歧义时，需要在其之前添加转义符`\\`

    Examples:
        >>> {cmd} {Op.ADD}炸鸡汉堡
        >>> {cmd} 炸鸡汉堡 {Op.ADD}肯德基 {Op.ADD}麦当劳 {Op.ADD}"Burger King"
        >>> {cmd} 炸鸡汉堡 {Op.REMOVE}2

        >>> {cmd} {Op.ADD}中式快餐
        >>> {cmd} 中式快餐 {Op.ADD}老乡鸡 {Op.ADD}真功夫

        >>> {cmd} {Op.ADD}吃什么
        >>> {cmd} 吃什么 {Op.ADD}[炸鸡汉堡] {Op.ADD}[中式快餐]
        >>> {cmd} 吃什么
        # 会从炸鸡汉堡和中式快餐的非引用内容中随机选择

    Note:
        - 慎用删除功能，仅创建者/管理员可用，删除后列表所有内容将被清空，**不可恢复**
        - 删除纯数字内容的列表项目时，必须通过**序号**指定以消解歧义
    """
