...
async def test_render_message():
    ...
    for i, (content, nickname, alignment, name) in enumerate(testcases):
        ...
        message = await MessageRender.create(
            avatar=avatar, content=msg, nickname=nickname, alignment=alignment
        )
    ...
    message_complex = await MessageRender.create(
        avatar=avatar, content=complex_msg, nickname="complex"
    )
    ...
    (await MessageRender.create_conversation(messages)).render().save(out / "all.png")
