...
def test_render_simulate_italic():
    RenderText.of(
        ...
        italic=True,
    ).render().save("render-test/render/pseudo_italic.png")


def test_render_text_italic_bold():
    ff = FontFamily(
        regular="data/static/fonts/arial.ttf",
    ...
    text = Paragraph.from_markup(
        ...
        default=default,
        styles=dict(
            i=TextStyle(italic=True), b=TextStyle(bold=True), cn=TextStyle(font=ff_cn)
        ),
...
def test_render_compare_sim_real_italic():
    ...
    ).render().save(out / "sim_vs_real.png")
